	...
	public void onKeyInput(InputEvent.KeyInputEvent event) {
		...
		if (KeyBindings.keyJ.isPressed()){
			...
			for(Step step: stepsToGoal){
				...
				if (null != step)
					System.out.println("(" + step.getLocation().xCoord + ", " + step.getLocation().yCoord + ", " + step.getLocation().zCoord + ") " + step.getType().toString());
				else
		...
		if (KeyBindings.keyU.isPressed()) {
			...
			Vec3 playerPos = Vec3.createVectorHelper(player.posX,player.posY - 2, player.posZ);
			List<Step> neighbors = worldInfo.getNeighbors(new Step(null, Step.StepType.Walk, 0, playerPos));
