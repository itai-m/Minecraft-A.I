...
class PathFinder {
	...
	public Queue<Step> findPath() {
		...
		Step step = new Step(null, StepType.Walk, 0, initPos);
		...
		while(worldInfo.getMinimalDistance(step.getLocation(), goal) > 0){ // H = 0
			...
			if(step.getTotalCost() < MAX_TOTAL_COST) {
				...
				for(Step neighbor : neighbors){
					if (null != neighbor && StepType.Unassinged != neighbor.getType())
					{
						steps.add(neighbor);
		...
		returnStepsList.addFirst(new Step(null, StepType.Walk, 0, initPos));
